...
public class AgentToolDescriptorTests
{
    ...
    public async Task ExecuteAsync_WithStaticMethod_InvokesSuccessfully()
    {
        ...
        // Act
        var result = await descriptors[0].ExecuteAsync(input, context, CreateIdentityContext(), CreateServiceProvider(), CancellationToken.None);
    ...
    [Fact]
    public async Task ExecuteAsync_InjectsIdentityContext()
    {
        // Arrange
        var descriptors = AgentToolDescriptor.FromType(typeof(ContextToolClass));
        var input = JsonSerializer.Deserialize<JsonElement>("{}");
        var context = CreateGrainContext();
        var identityContext = CreateIdentityContext(Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"));

        // Act
        var result = await descriptors[0].ExecuteAsync(input, context, identityContext, CreateServiceProvider(), CancellationToken.None);
    ...
    [Fact]
    public async Task ExecuteAsync_WithDIConstructedClass_InjectsService()
    {
        // Arrange
        var descriptors = AgentToolDescriptor.FromType(typeof(DiToolClass));
        var input = JsonSerializer.Deserialize<JsonElement>("""{"value": "test"}""");
        var context = CreateGrainContext();
        var serviceCollection = new ServiceCollection();
        serviceCollection.AddSingleton(new DiToolClass.IDependency_Impl("injected"));
        serviceCollection.AddSingleton<DiToolClass.IDependency>(sp => sp.GetRequiredService<DiToolClass.IDependency_Impl>());
        var serviceProvider = serviceCollection.BuildServiceProvider();

        // Act
        var result = await descriptors[0].ExecuteAsync(input, context, CreateIdentityContext(), serviceProvider, CancellationToken.None);

        // Assert
        Assert.False(result.IsError);
        Assert.Equal("injected:test", result.Content);
    }
    ...
    private static IIdentityContext CreateIdentityContext(Guid? userId = null)
    {
        var mock = new Mock<IIdentityContext>();
        mock.Setup(x => x.UserId).Returns(userId ?? Guid.Parse("11111111-1111-1111-1111-111111111111"));
    ...
    public class ContextToolClass
    {
        ...
        public ToolResult Run(IIdentityContext identityContext) => ToolResult.Success(identityContext.UserId.ToString());
    }
    ...
    public class DiToolClass
    {
        ...
        public class IDependency_Impl : IDependency
        {
            public string Value { get; }
            public IDependency_Impl(string value) => Value = value;
        }

        private readonly IDependency _dependency;

        public DiToolClass(IDependency dependency) => _dependency = dependency;

        [AgentTool("di_tool")]
        [Description("Tool with DI dependency")]
        public ToolResult Run(string value) => ToolResult.Success($"{_dependency.Value}:{value}");
    }
