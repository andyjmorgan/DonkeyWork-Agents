...
public sealed class SwarmAgentManagementTools
{
    ...
    {
        if (context is null || identityContext is null)
        {
            return ToolResult.Error("GrainContext and IIdentityContext are required.");
        }
    ...
    private static IAgentRegistryGrain GetRegistry(GrainContext context, IIdentityContext identityContext)
    {
