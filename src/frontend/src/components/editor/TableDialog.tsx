...
export function TableDialog({ open, onOpenChange, onInsertTable }: TableDialogProps) {
  ...
  // Update headers array when cols change
  useEffect(() => {
    setHeaders(prev => {
  ...
  const adjustCols = (delta: number) => {
    setCols(prev => Math.min(MAX_COLS, Math.max(MIN_COLS, prev + delta)))
  }
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    ...
                    max={MAX_COLS}
                    value={cols}
                    onChange={(e) => setCols(Math.min(MAX_COLS, Math.max(MIN_COLS, parseInt(e.target.value) || MIN_COLS)))}
                    className="w-16 text-center"
                  />
                  <Button
                    variant="outline"
