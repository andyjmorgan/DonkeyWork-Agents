...
using CodeSandbox.Executor.Models;
...
public class StdioBridge : IDisposable
{
    ...
    /// <summary>
    /// Internal event type for streaming startup progress.
    /// </summary>
    public sealed record StartEvent(string EventType, string Message, string? Stream = null, int? ExitCode = null, int? Pid = null, double? ElapsedSeconds = null);

    public async Task StartAsync(
        string command,
        string[] arguments,
        string[] preExecScripts,
        int timeoutSeconds,
        string? workingDirectory,
        ChannelWriter<StartEvent>? events,
        CancellationToken cancellationToken)
    {
        ...
        void Emit(string eventType, string message, string? stream = null, int? pid = null)
        {
            events?.TryWrite(new StartEvent(eventType, message, stream, Pid: pid,
        ...
        try
        {
            foreach (var script in preExecScripts)
            {
                ...
                await RunPreExecScriptAsync(script, workingDirectory, events, cancellationToken);
                Emit("pre_exec_complete", $"Completed: {script}");
            }

            // Launch the MCP stdio server process
            var commandDisplay = $"{command} {string.Join(" ", arguments)}";
            _logger.LogInformation("Launching MCP server: {Command} {Arguments}",
                command, string.Join(" ", arguments));
            Emit("process_starting", $"Launching: {commandDisplay}");
            LaunchMcpProcess(command, arguments, workingDirectory, events);

            Emit("process_started", "Process started", pid: _mcpProcess?.Id);
            ...
            await WaitForMcpReadyAsync(timeoutSeconds, events, cancellationToken);
            ...
            Emit("ready", "MCP server is ready", pid: _mcpProcess?.Id);
        }
        catch (Exception ex)
        {
    ...
    private async Task WaitForMcpReadyAsync(int timeoutSeconds, ChannelWriter<StartEvent>? events, CancellationToken cancellationToken)
    {
        var startTime = DateTime.UtcNow;
        ...
        {
            events?.TryWrite(new StartEvent(eventType, message, stream,
                ElapsedSeconds: (DateTime.UtcNow - startTime).TotalSeconds));
        }
    ...
    private async Task RunPreExecScriptAsync(string script, string? workingDirectory, ChannelWriter<StartEvent>? events, CancellationToken cancellationToken)
    {
        ...
        var stdoutTask = Task.Run(async () =>
        {
            while (await process.StandardOutput.ReadLineAsync(cancellationToken) is { } line)
            {
                ...
                events?.TryWrite(new StartEvent("pre_exec_output", line, "stdout"));
            }
        }, cancellationToken);
        ...
        var stderrTask = Task.Run(async () =>
        {
            while (await process.StandardError.ReadLineAsync(cancellationToken) is { } line)
            {
                ...
                events?.TryWrite(new StartEvent("pre_exec_output", line, "stderr"));
            }
        }, cancellationToken);
    ...
    private void LaunchMcpProcess(string command, string[] arguments, string? workingDirectory, ChannelWriter<StartEvent>? events)
    {
        var workDir = ResolveWorkingDirectory(workingDirectory);
        ...
        _logger.LogDebug("Starting process: {Command} {Args} in {WorkingDir}",
            command, argumentString, workDir);
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                while (await _mcpProcess.StandardError.ReadLineAsync() is { } line)
                {
                    ...
                    events?.TryWrite(new StartEvent("mcp_stderr", line, "stderr"));
                }
            }
            catch
            {
    ...
    private static string ResolveWorkingDirectory(string? workingDirectory)
    {
        ...
        return Directory.Exists("/home/user")
            ? "/home/user"
    ...
    internal static bool IsNotification(string json)
    {
        try
        {
    ...
    internal static string? ExtractMethod(string json)
    {
        try
        {
            using var doc = JsonDocument.Parse(json);
    ...
    internal static bool IsJsonRpcResponse(string line)
    {
        try
        {
    ...
    /// </summary>
    internal static bool TryParseJsonRpc(string line, out bool hasId, out string? id, out bool isResponse, out bool hasMethod)
    {
        hasId = false;
        id = null;
        isResponse = false;
